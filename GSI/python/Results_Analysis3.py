...
class Results_Analysis:
    ...
    def __init__(self, prefix: str, psi1: float, psi2: float, min_detect: float, psi3: float, detect_model: int, ref: pd.DataFrame, threshold: Union[int, None] = None, max_edges: Union[int, None] = None, spectra_error_rate: Union[float, None] = None, false_edges: Union[int, None] = None, synthetic_data: bool = False):
        self.prefix = prefix
        self.psi1 = psi1
        self.psi2 = psi2
        ...
        self.upper_edges = pd.read_csv(self.models_dir / f"upper_edges_{prefix}_{detect_model}_{threshold}_{max_edges}_{psi1:.1f}_{psi2:.1f}_{min_detect:.2f}_{psi3:.2f}.csv")
        self.lower_edges = pd.read_csv(self.models_dir / f"lower_edges_{prefix}_{detect_model}_{threshold}_{max_edges}_{psi1:.1f}_{psi2:.1f}_{min_detect:.2f}_{psi3:.2f}.csv")
        self.selected_peptides = pd.read_csv(self.sol_dir / f"ident_results_{prefix}_{detect_model}_{threshold}_{max_edges}_{psi1:.1f}_{psi2:.1f}_{min_detect:.2f}_{psi3:.2f}.csv")
        self.selected_edges = pd.read_csv(self.sol_dir / f"select_results_{prefix}_{detect_model}_{threshold}_{max_edges}_{psi1:.1f}_{psi2:.1f}_{min_detect:.2f}_{psi3:.2f}.csv")
        self.ref = ref
        self.sol = pd.read_csv(self.sol_dir / f"results_{prefix}_{detect_model}_{threshold}_{max_edges}_{psi1:.1f}_{psi2:.1f}_{min_detect:.2f}_{psi3:.2f}.csv")
        ...
        self.protein_to_spectra = pd.merge(self.protein_to_spectra, self.selected_edges, left_on=["protein_id", "peptide_id", "rank"], right_on=["protein", "peptide", "rank"], how="left", indicator="selected_edge")
        self.protein_to_spectra.drop(["protein", "peptide"], inplace=True, axis=1)
        self.protein_to_spectra["selected_edge"] = np.where(self.protein_to_spectra["selected_edge"] == "both", True, False)
    ...
    def draw_graph(self, select_func, figsize=(30, 10), dpi=250, with_labels=True, label="label_1", with_edges_labels=False, add_prot=0) -> None:
        ...
        for index, row in self.protein_to_spectra.dropna()[["accession", "peptide_id", "protein_prediction", "protein_truth", "prediction_category", "Prob", "selected_edge"]].drop_duplicates().iterrows():
            ...
            G.add_edge(protein, peptide, color=row["Prob"], style=styles[row["selected_edge"]])
