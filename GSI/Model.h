...
struct Solution {
    ...
    */
    void Save(std::vector<Spectrum*> spectra, const std::string file_name, bool overwrite = false, bool save_proteins = true, bool save_ident = true) const;
    /*
    * Enregistre la solution dans un fichier csv.
    ...
    */
    void Save(std::vector<Spectrum*> spectra, std::ofstream& output_file, bool save_proteins = true, bool save_ident = true) const;

    /*
    * Enregistre la solution dans le fichier de log fourni.
    * Les numéros d'accession des protéines sont utilisés comme identifiants lors de l'enregistrement des protéines identifiées.
    */
    void Save(std::vector<Spectrum*> spectra, std::ofstream& output_file, const std::vector<Protein*>& proteins, bool save_proteins = true, bool save_ident = true) const;
};
...
public:
    ...
    */
    int Solve(const float psi1 = 0.5f, const float psi2 = 0.5f, const float psi3 =  0.5f);
    /*
    * Calcule une solution pour le mod�le courant. psi1 correspond au coefficient de l'objectif sur les Deltas, psi2 correspond au coefficient de l'objectif sur les ar�tes spectre-peptide.
